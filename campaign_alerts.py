"""
Phát hiện "đột biến" (spike) và "giảm dần" (gradual decline) cho CPI/ARPU D0
("LTV D0")/ROAS D0 theo TỪNG CAMPAIGN — dùng riêng dữ liệu Adjust (KHÔNG cần
ghép BigQuery, nên áp dụng được cho MỌI channel, không chỉ Meta/Facebook).

QUAN TRỌNG — vì sao có cả CPI, ARPU D0 LẪN ROAS D0 (không chỉ CPI+ROAS): user
chỉ ra (16/09/2026) CPI+ROAS thôi CHƯA ĐỦ để biết ROAS biến động là do CHI PHÍ
đắt lên hay do GIÁ TRỊ NGƯỜI DÙNG (LTV) tụt xuống — vì ROAS D0 = ARPU D0 ÷ CPI,
1 mình ROAS D0 không tách được 2 nguyên nhân này. Theo dõi riêng CẢ 3 để biết
CHÍNH XÁC lever nào đang biến động.
...
— chỉ báo theo ĐÚNG CHIỀU XẤU: CPI tăng, hoặc ARPU D0/ROAS D0 giảm, quá
`decline_threshold_pct`%.

Cả 2 đều tính CPI/ARPU D0/ROAS D0 ĐÚNG CÁCH mỗi khi gộp nhiều ngày/nhiều dòng
— cộng dồn installs/network_cost (và revenue suy từ roas_ad_d0 × network_cost)
rồi chia lại, KHÔNG trung bình cộng trực tiếp cột CPI/ARPU/ROAS D0 (xem quy
tắc đã chốt trong adjust_client.py — sai số 3-4% nếu làm sai).

`min_installs`: lọc bớt campaign quá ít traffic trong CẢ khoảng ngày đã kéo —
tránh báo động giả (mẫu nhỏ, 1-2 install cũng đủ làm CPI/ARPU/ROAS nhảy vọt vô nghĩa).
"""
...
def build_campaign_daily(adjust_df: pd.DataFrame, product_id: str) -> pd.DataFrame:
    ...
    grouped["arpu_d0"] = grouped["_revenue_d0"] / grouped["installs"].replace(0, pd.NA)
...
def _weighted_cpi_roas(g: pd.DataFrame):
    ...
    arpu = (revenue_d0 / installs) if installs else None
    return cpi, roas, arpu
...
def analyze_campaigns(
    ...
    for campaign, g in daily_df.groupby("campaign"):
        ...
        latest = g.iloc[-1]
        cpi_spike_pct = roas_spike_pct = arpu_spike_pct = None
        if len(g) >= spike_days + 1:
            baseline = g.iloc[-(spike_days + 1):-1]
            base_cpi, base_roas, base_arpu = _weighted_cpi_roas(baseline)
            ...
            if base_arpu is not None and base_arpu != 0 and pd.notna(latest["arpu_d0"]):
                arpu_spike_pct = (latest["arpu_d0"] - base_arpu) / base_arpu * 100

        cpi_decline_pct = roas_decline_pct = arpu_decline_pct = None
        if len(g) >= decline_window * 2:
            recent = g.iloc[-decline_window:]
            previous = g.iloc[-decline_window * 2 : -decline_window]
            recent_cpi, recent_roas, recent_arpu = _weighted_cpi_roas(recent)
            prev_cpi, prev_roas, prev_arpu = _weighted_cpi_roas(previous)
            ...
            if prev_arpu is not None and prev_arpu != 0 and recent_arpu is not None:
                arpu_decline_pct = (recent_arpu - prev_arpu) / prev_arpu * 100
        ...
        if arpu_spike_pct is not None and arpu_spike_pct <= -spike_threshold_pct:
            flags.append("🔴 LTV (ARPU D0) tụt đột biến")
        elif arpu_spike_pct is not None and arpu_spike_pct >= spike_threshold_pct:
            flags.append("🟢 LTV (ARPU D0) tăng đột biến")
        ...
        if arpu_decline_pct is not None and arpu_decline_pct <= -decline_threshold_pct:
            flags.append("🟠 LTV (ARPU D0) giảm dần")
        ...
        rows.append(
            {
                ...
                "LTV (ARPU D0) gần nhất": round(latest["arpu_d0"], 4) if pd.notna(latest["arpu_d0"]) else None,
                "LTV % lệch vs TB 7 ngày trước": round(arpu_spike_pct, 1) if arpu_spike_pct is not None else None,
                ...
                "LTV % đổi (7 ngày vs 7 ngày trước đó)": round(arpu_decline_pct, 1) if arpu_decline_pct is not None else None,
